# RidgeCode PROJECT-STATE(2026-07-23)
...
- **2026-07-23(本轮)**:无 key 可开 TUI —— TUI 不再要求先配 key,ScriptedProvider 兜底提示用 /login;TUI /login 页新增 **claude-oauth 行**(浏览器授权 → 贴 code#state → 就地交换 token → 热切 bearer provider),CLI 与 TUI 双入口共用 provider::oauth 纯核;RIDGE_PROXY env(env > config.proxy > 直连)。
- **验证证据**:`cargo test --workspace`(212 tests)+ `cargo fmt --all --check` + `cargo clippy --workspace --all-targets -- -D warnings` 全 exit 0(2026-07-23 实跑)。

## C. 能力对照与差距(距「pi-agent 式多订阅接入」)

对标 pi-agent(pi-coding-agent / @mariozechner/pi-ai)的订阅接入架构:OAuth PKCE 授权模块 + Token 存储刷新模块 + Bearer/SSE 流式客户端,`/login` 内选择订阅源(Claude Pro/Max、ChatGPT Plus/Pro (Codex)、…)即可绑定。

| 能力 | RidgeCode 现状 | 差距 |
|---|---|---|
| API-key 登录(14 内置 preset) | ✅ CLI `login <id>` + TUI /login 页,先校验后落盘 | — |
| Claude Pro/Max 订阅 OAuth | ✅ CLI `login --claude` + TUI claude-oauth 行;PKCE 纯核;oauth.json;自动刷新;启动回退 | 活端点校验依赖用户实跑(ToS 灰区,常量集中可覆盖) |
| **ChatGPT Plus/Pro(Codex)订阅 OAuth** | ❌ 无 | pi-agent 已有;OpenAI auth 端点 PKCE 流 + codex 模型;oauth.json 已预留多 provider 键位 |
| 订阅档一等公民化 | ⚠️ 部分 | OAuth 凭据是「key 全无时的回退」,不是 providers[] 命名档;多订阅并存/热切换(/provider 页列订阅档)未做 |
| 其他订阅源(Gemini 等) | ❌ 无 | 优先级待定 |
| 本地代理桥接(openai-oauth 型) | ❌ 无(可经 base_url 覆盖间接支持) | 是否内置待定 |

## D. 开放问题(请 NotebookLM 结合全部来源定夺)

1. **Codex OAuth 接入设计**:复用 provider::oauth 纯核抽象(把 anthropic 常量泛化为 per-provider OAuthConfig)还是并列新模块?ChatGPT Plus 的 token 端点/scope/SSE wire 与 Anthropic 差异如何隔离在 trait 后?
2. **订阅档一等公民化**:oauth.json 凭据如何进 providers[] 体系(kind=oauth?),使 /provider 页可列可切、多订阅并存?与「key 全无回退」的兼容路径?
3. **迭代排序**:Codex OAuth → 订阅档一等公民化 → 其他订阅源,这个顺序是否最优?每步的确定性验收信号建议?
4. TUI 遗留 bug(底部多余状态条/状态条换行/Shift+Enter 失效/光标卡首行,iter-47 延后)与订阅主线如何穿插?
