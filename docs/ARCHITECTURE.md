...
providers 命名档(kind/model/base_url/**key_env** —— 密钥只走 env 绝不落盘)/ budget_tokens / skip_danger / 多 `mcp{name,cmd,args}` / skills_dir。`RIDGE_CONFIG` 指路径;TUI `/config set` 可持久化回写。
