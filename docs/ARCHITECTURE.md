...
- **`ridgecode login` 子命令**(iter-37,`main` 在解析任务前拦下):`login [--list]` 列内置表;`login <id> [KEY] [--model M] [--name N] [--no-default]` → key 写 auth.json(缺 KEY 从 stdin 读,**绝不回显/落 argv**)+ 据 preset 写档 +(默认)设顶层默认。TUI `/login <id> <key>` 同语义并即时热切。**OAuth 订阅登录仅留接缝**(auth.json 值可扩对象),本轮不落地。
