...
- **`main`** 分流:`login` 子命令(iter-37,内置供应商一键接入,见 §7)在解析任务前拦下;否则有 key(`RIDGE_API_KEY`/顶层内联/顶层 `key_env`→auth/`providers[]` 档)→ 真实路径,无 → 离线脚本 demo。
...
## 7. config(`~/.ridge/config.json`,env 覆盖)+ auth 密钥库(iter-37)
...
- **`~/.ridge/auth.json` 密钥库**(iter-37):`login` 存 API key 处,**独立于 config,key 不进 config**;按 `key_env` 名索引(纯核 `auth_parse`/`auth_upsert`/`auth_get`,坏/空不崩;字符串值,OAuth 对象值前向跳过);写盘 best-effort chmod 600(unix)。`RIDGE_AUTH` 可指路径。
- **key 解析新增一档**(`resolve_key_env(name,&auth)` = env[name] 非空 或 auth[name];`ProviderProfile::resolve_key_with(&auth)` = 内联 `api_key` > 该档)。启动 `real_provider` 顺序:`RIDGE_API_KEY` env → 顶层内联 `api_key` → 顶层 `key_env`→(env/auth)→ `providers[]` 迭代(auth-aware)。`build_agents`/TUI `switch_provider`/`current_api_key` 同 auth-aware。
- **`ridgecode login` 子命令**(iter-37,`main` 在解析任务前拦下):`login [--list]` 列内置表;`login <id> [KEY] [--model M] [--name N] [--no-default] [--no-verify]` → key 写 auth.json(缺 KEY 从 stdin 读,**绝不回显/落 argv**)+ 据 preset 写档 +(默认)设顶层默认。**OAuth 订阅登录仅留接缝**(auth.json 值可扩对象),本轮不落地。
...
- **Hook 系统(iter-40)**:config `hooks: [HookCfg{event, matcher, command, blocking}]` + `notify`。四事件 `pre_tool`/`post_tool`/`session_start`/`stop`;`matcher`=工具名子串(`*_tool`,缺=全部)。**触发点**:`execute_tool_call` 前后串 pre/post_tool(`run_pre_tool_hooks` 的 blocking hook 非 0 退出 → BLOCKED 拦下工具;`run_post_tool_hooks` fire-and-forget),`main` 启动 `session_start`、各任务毕 `stop`(tui done / run_once / headless)。hook 命令经带 `.env()` 的 Command 跑(注入 `RIDGE_TOOL`/`RIDGE_TOOL_ARG`,**不设全局 env**,BSP 并发安全)。**内置(总是安全)**:会话审计留痕(`audit`→`~/.ridge/audit.log`,前置 epoch 秒)、任务完成响铃(`notify`→`\x07`);格式化/危险确认作为 config 示例 hook 随 `install.ps1` 下发。全局 `HOOKS: OnceLock`/`NOTIFY: AtomicBool`(set-once,与 jailbreak/dynamic-commands 先例一致);纯核 `hooks_for_event`/`audit_line` 可单测。
