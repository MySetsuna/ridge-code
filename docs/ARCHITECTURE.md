...
`build_llm_agent_full(provider, mcp, approver, skills, token_bus, agents, read_only)` → `build_core` 装配 reason→act→verify；TUI 运行路径使用 `build_llm_agent_full_with_steer(..., SteerBus)`:

- **live steer**:`SteerBus` 对用户输入有界为 32 条/消息 32768 字符；provider 请求中的已取消息与新输入合计最多 64 条，避免失败回队时静默丢失。TUI 的 `Ctrl+Shift+Enter` 或 `/steer <guidance>` 只入队，不取消当前 provider 请求；`reason` 在下一次请求前一次性消费并以独立 user guidance 消息注入。provider 失败时消息按原顺序回队，消息会在 assistant/tool-call 对之前落入 history，保持协议顺序；当前回合已结束时 TUI 自动排入 follow-up。中断会清空 steer 队列。
...
agent 侧 `resolve_mcp`:多 client 各自握手 + 列工具 → 归一化 `McpTools{specs, router}`;**降级不崩** —— 单 server 失败跳过。config 支持多 `mcp` 声明 + 兼容旧 env `RIDGE_MCP_CMD`。TUI `/mcp` 另经 `host_mcp_servers` 只读解析宿主 `CODEX_HOME/config.toml` 的 `mcp_servers`，列出已安装/已配置 companion 但不隐式启动宿主条目。
...
- **TUI v2 动作与性能层**:`tui/keymap.rs` 以稳定 action id 统一默认键位、配置覆盖、输入路由与帮助投影；`Ctrl+P` 打开内置/动态 slash command 搜索面板，`F1`/`/help` 展示当前生效键位。`Config.keybindings` 整组原子校验，安全/编辑键不可覆盖。Snapshot opt-in 遥测除 draw 外还记录 event-to-frame、serialize/write 与 payload bytes，Completion+Resize ConPTY 对其设置硬预算；未启用 Snapshot 时仍无相关分配与 I/O。
