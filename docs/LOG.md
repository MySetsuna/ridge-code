...
- **对抗评审**:notes 多为 **iter-12 时代**所拟,须对当下(iter-20)现实重估。**采纳**其自陈根因「模型很蠢=驾驭工程没把物理约束讲透」;**押后**巨幅 TUI 重写(打磨非正确性/量大/难无抖测/用户在场紧迫度低);**重申驳回** Saga 自动回滚(数据丢失+违 preserve-mistakes);claw-tsaver 折叠的 token 实质已由 iter-19 `bound_observation` 内核截断解决。
- **做了什么**:①**系统提示词硬化** `BASE_SYSTEM` 注入三条 harness 物理契约(terse 利缓存):大输出被截断→用 ranged read_file/search 勿反复巨读(iter-19)、勿删/清空测试[拦=失败](前置对齐 iter-18)、`signal_write` 沉淀复利(iter-16/20)——把 iter-17/19/20 造出的**新物理事实讲给模型**。②**工具调用鲁棒** `execute_tool_call` 未知工具默认臂 `"unknown tool"`→`"tool error: 未知工具 ..."`(含 " error:"),幻觉工具由此喂 iter-18 熔断计数+失败信号,不再静默空转。
- **验收**:`cargo test --workspace`=全绿(agent lib 66→**68**,+2:harness 契约短语在位、未知工具归 error+失败信号),clippy `-D warnings`、fmt 净。
- **成果**:模型↔harness 交互契约对齐,减无效轮(反复巨读/幻觉工具/删测试)→ 省 token、少跑飞。
- **押后专轨**:TUI 交互重构(视口/模态状态机治「滚动即拒绝」/滚动/补全菜单/审美)—— 需独立 iteration + 用户在场做物理验收;纯状态转移可单测,渲染/滚动难无抖测。
- **下一步**(俟用户):启动 TUI 交互专轨,或工具调用参数容错,或系统提示词真 LLM 实测(外环人验收)。
