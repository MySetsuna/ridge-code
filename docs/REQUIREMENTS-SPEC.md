...
- Status:`ACTIVE`
- Version:`v0.1.0`
- Behavior:`agent 对只读探索设置可观测、有界的阶段推进与停滞检测；连续探索无进展时能确定性转入编写/澄清/阻塞路径，不因重复读文件无限循环至退出；工具调用、阶段、停滞原因和下一步对用户可见。TUI 将不同终端产生的 Backspace、Enter、鼠标与功能键序列统一归一化为稳定事件，PowerShell、Windows 传统控制台、Windows Terminal、常见 ANSI/VT 终端及可复现 PTY 中编辑、提交、取消、面板导航和鼠标操作一致可用。Kitty keyboard protocol 仅按 `tui::terminal::terminal_keyboard_policy` 的显式正证据协商；终端名称或 `VTE_VERSION` 不足以证明 PTY 保留 KKP，故默认走 legacy，仅经验证后用 `RIDGE_TUI_KITTY=1` 显式启用；Windows 原生输入、VS Code/xterm.js、Apple Terminal、JetBrains、旧/新 VTE、未知 multiplexer/终端均以 `Alt+Enter/Ctrl+J` 维持换行回退，显式 `RIDGE_TUI_KITTY=0` 可强制关闭。`/goal '需求'` 解析为完整 goal 标题并持久化，空参数、引号/空白、重复运行与错误状态有明确提示，不破坏既有 `/goal` 子命令。已提交历史输出通过 `Terminal::insert_before` 写入终端原生 scrollback；Live 视口只保留当前活动帧，历史可由终端滚动、选取、搜索回溯，不以反复 ratatui 重绘替代 scrollback。`
...
- Acceptance:`新增 agent 阶段转移、重复探索、无进展、写入触发、StepLimit/timeout/cancel 与诊断测试，证明探索预算、进展信号和写入路径均有界且无重复读文件死循环；新增按原始字节/事件构造的 Backspace(0x08/0x7f)、Enter(0x0a/0x0d/CSI/VT)、功能键、鼠标序列和 release/press 过滤测试，并完成可复现 Windows/PTY smoke；`terminal_keyboard_policy` 纯矩阵须覆盖已知/未知/受限终端与显式覆盖，状态提示须与实际协商结果一致；`windows-pty-e2e.ps1 -InputFixture` 还须证明 BS/DEL/TAB/LF 与 raw bracketed paste 经真实 ConPTY 路由且输出受 `-MaxOutputBytes` 有界，并以同一次物理写入的 bracketed paste + CRLF 证明恰生成一个 Paste、一个 Enter、零输入残留且仅启动一次任务；`Event::Paste` 与 raw bracketed fallback 回归须证明 CRLF/CR 归一、CSI/OSC 残尾不泄入输入框；Crossterm fallback 须跨批次过滤 SGR mouse/focus 报告，同时保持普通 `[I`/`[O`、空格、Enter、Tab 原义；新增 `/goal '需求'`、带空白/引号、空参数、重复执行、持久化 round-trip 测试；新增 scrollback/Live 回归，证明提交历史进入 `insert_before` 原生 scrollback，后续刷新不覆盖/重复历史，Live 仅承载当前帧且历史可滚动回溯；通过 cargo fmt --all --check、npm run spectree:check、cargo test --workspace --locked、cargo clippy --workspace --all-targets --locked -- -D warnings、cargo build --workspace --locked、git diff --check，并运行现有 Sonar quality gate（如其入口已配置）。`
