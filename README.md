...
新 PC 无需 Rust/Cargo：复制当前平台的一条命令，安装器会从 GitHub Release 下载对应归档、校验 SHA256，并把 `ridgecode` 加入用户 PATH。当前稳定版为 `v0.5.30`；每个归档内同时带有完整 `README.md` 与安装脚本，便于离线转交和审计。
...
# 可复现安装：固定脚本与 Release 版本 v0.5.30
$s = irm https://raw.githubusercontent.com/MySetsuna/ridge-code/v0.5.30/scripts/install.ps1
& ([scriptblock]::Create($s)) -Version v0.5.30
...
# 可复现安装：固定脚本与 Release 版本 v0.5.30
curl -fsSL https://raw.githubusercontent.com/MySetsuna/ridge-code/v0.5.30/scripts/install.sh | sh -s -- --version v0.5.30
...
当前 Release：[v0.5.30](https://github.com/MySetsuna/ridge-code/releases/tag/v0.5.30)。手动下载时按平台选择：
...
~~~bash
gh release download v0.5.30 --repo MySetsuna/ridge-code --pattern 'ridgecode-*'
~~~
