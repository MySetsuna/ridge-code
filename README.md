...
新 PC 无需 Rust/Cargo：复制当前平台的一条命令，安装器会从 GitHub Release 下载对应归档、校验 SHA256，并把 `ridgecode` 加入用户 PATH。当前稳定版为 `v0.5.29`；每个归档内同时带有完整 `README.md` 与安装脚本，便于离线转交和审计。
...
# 可复现安装：固定脚本与 Release 版本 v0.5.29
$s = irm https://raw.githubusercontent.com/MySetsuna/ridge-code/v0.5.29/scripts/install.ps1
& ([scriptblock]::Create($s)) -Version v0.5.29
...
# 可复现安装：固定脚本与 Release 版本 v0.5.29
curl -fsSL https://raw.githubusercontent.com/MySetsuna/ridge-code/v0.5.29/scripts/install.sh | sh -s -- --version v0.5.29
...
当前 Release：[v0.5.29](https://github.com/MySetsuna/ridge-code/releases/tag/v0.5.29)。手动下载时按平台选择：
...
~~~bash
gh release download v0.5.29 --repo MySetsuna/ridge-code --pattern 'ridgecode-*'
~~~
...
直接运行 ridgecode 即进入 inline TUI。输出会以不透明表面写入原生 scrollback，避免终端透明背景或壁纸干扰阅读；底部仅保留紧凑的状态与输入命令栏。

可用 `/theme dark|light|auto` 切换并持久化配色（`auto` 仅在终端提供可靠背景提示时跟随，否则安全回退深色）。`Alt+R`、`Alt+T`、`Alt+G` 与 `Alt+I` 保留为当前会话记录的搜索与导航入口。
