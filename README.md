...
底层是纯 Rust 的 LangGraph 风格有状态图引擎，外层接 provider、工具、MCP、Skills 与 TUI。TUI 以终端原生 scrollback 为唯一任务时间线：任务、推理、工具调用/完整结果、答案与结束状态按阶段写入，可直接滚动、复制与搜索。

## 先跑起来

### 其他 PC：通过 GitHub 快速安装（无需 Rust/Cargo）

新 PC 无需 Rust/Cargo：复制当前平台的一条命令，安装器会从 GitHub Release 下载对应归档、校验 SHA256，并把 `ridgecode` 加入用户 PATH。当前稳定版为 `v0.5.32`；每个归档内同时带有完整 `README.md` 与安装脚本，便于离线转交和审计。
...
# 可复现安装：固定脚本与 Release 版本 v0.5.32
$s = irm https://raw.githubusercontent.com/MySetsuna/ridge-code/v0.5.32/scripts/install.ps1
& ([scriptblock]::Create($s)) -Version v0.5.32
...
# 可复现安装：固定脚本与 Release 版本 v0.5.32
curl -fsSL https://raw.githubusercontent.com/MySetsuna/ridge-code/v0.5.32/scripts/install.sh | sh -s -- --version v0.5.32
...
安装器首次运行会生成 `~/.ridge/config.json`（Windows 为 `%USERPROFILE%\.ridge\config.json`）与 `config.example.json`；骨架使用通用 OpenAI 端点，按下方 provider 档案改为第三方兼容服务即可。填入 API Key、设置 `RIDGECODE_API_KEY` 或在 TUI 使用 `/login` 后即可启动真实模型。安装完成后若当前 shell 尚未刷新 PATH，请新开终端。
...
Linux / macOS 将 `$env:RIDGECODE_API_KEY` 改为 `export RIDGECODE_API_KEY="your-key"`。安装器支持 Windows x86_64、Linux x86_64/ARM64、macOS Intel/Apple Silicon；Windows ARM64 可改用 WSL 或从源码构建。企业或审计环境可先下载 `scripts/install.ps1` / `scripts/install.sh` 检查内容，再用本地脚本执行；安装器也支持 `-Dir`、`--dir` 自定义目录。Windows 的 `-NoPath -NoConfig` 与 Unix 的 `--no-config` 可用于 CI、排障或不希望修改用户状态的场景。
...
当前 Release：[v0.5.32](https://github.com/MySetsuna/ridge-code/releases/tag/v0.5.32)。手动下载时按平台选择：
...
~~~bash
gh release download v0.5.32 --repo MySetsuna/ridge-code --pattern 'ridgecode-*'
~~~

归档内包含二进制、完整 `README.md` 与对应安装脚本；解压后可用 `--local` / `-Local` 安装，仍会校验归档旁的 `.sha256`。若组织策略禁止 `curl | sh` 或 `irm | iex`，先下载脚本并审阅，再按上面的“可复现安装”方式从本地调用；若 Release 下载被策略阻断，使用本节的 `gh release download` 离线传递归档。
...
ridgecode --yolo "任务"                自动批准工具，并允许本次会话写入 cwd 外；灾难命令仍硬拦截
...
直接运行 ridgecode 即进入 inline TUI。输出写入原生 scrollback，并保留终端自己的背景或透明效果；底部仅保留紧凑的状态与输入命令栏。

可用 `/theme dark|light|auto` 切换并持久化古罗马风格的前景配色（`auto` 仅在终端提供可靠背景提示时跟随，否则安全回退深色）。`Alt+R`、`Alt+T`、`Alt+G` 与 `Alt+I` 保留为当前会话记录的搜索与导航入口。

### Answer、Reasoning 与工具

- Answer 展示模型实际回答；收到的 reasoning_content 单独保存为 Reasoning，不生成或伪造隐藏思考，并在阶段完成后写入 scrollback。
...
2. 用终端自身的滚轮或历史查看、选择并复制一段 Answer 或完整工具结果；再用当前终端绑定的原生搜索快捷键搜索其中的英文、CJK 与 emoji 文本。复制内容不应包含 ANSI 转义片段。需要定位某一类事件时按 `Alt+T`、`Alt+R` 或 `Alt+G`。
...
| /exit、/quit | 退出 TUI |
| /sessions | 列出当前 cwd 保存的会话 |
| /resume | 列出当前 cwd 会话；用 `/resume <id>` 恢复指定会话 |
| /reset | 清空当前上下文并保存空会话 |
...
/config set 允许持久化：provider、model、base_url、budget_tokens、skills_dir、skip_danger、status_bar、theme、allow_jailbreak、proxy。结构化字段（如 mcp、providers、hooks）请直接编辑 JSON。旧配置中的 `ui_density` 会被安全忽略。
...
A2A 是独立于 MCP 的协作协议。业务消息统一为版本化 `AgentEnvelope`，传输使用 newline JSON-RPC；本地调用仍可使用 in-process transport，跨进程 peer 由下列 CLI 入口承载：

~~~powershell
# 启动只读 peer：stdio 输入输出是协议帧，日志只写 stderr
ridgecode a2a serve --id ridgecode-worker

# 由 caller 启动一个 peer，完成一次有界只读任务
ridgecode a2a call --peer ridgecode --peer-arg a2a --peer-arg serve --peer-arg --once `
  --task "inspect the repository" --to ridgecode-worker --budget 15
...
read_file、search、web_search、fetch_url、todo_write、signal_write 与 dispatch_agent 不走用户审批；写文件、编辑和 shell 默认显示 diff/权限门。危险命令、受保护路径始终硬拦。--yolo 及其跳过权限别名（包括 `RIDGECODE_SKIP_PERMISSIONS=1` 和 `skip_danger=true`）会自动批准普通工具，并在本进程启用 cwd 外写入；不能绕过灾难拦截、受保护路径或 read-only。--read-only 从工具提供阶段和执行阶段双重拒绝副作用工具及 MCP。
...
Missing sonar-scanner or SONAR_TOKEN is a hard failure for this standalone
quality gate; the scan cannot be skipped there. The release workflow blocks on
