...
//!
//! 注:各家 base_url / kind / key 环境变量是稳定事实;**内置示例模型 ID 会随各家更新漂移**,
//! 用 `ridge-code models <name> --online` 从 models.dev 拉取真实当前模型;`init` 可填任意 model id。
...
pub struct CatalogEntry {
    ...
    pub free: bool,
    /// 该供应商在 models.dev 的 provider id(用于 `--online` 实时拉模型);空 = 无在线目录(本地)。
    pub modelsdev_id: &'static str,
    /// 一句话说明。
    pub note: &'static str,
    /// 内置示例模型 ID(非权威,可能更新;`--online` 拉真实的)。
...
pub fn catalog() -> &'static [CatalogEntry] {
    &[
        ...
        CatalogEntry {
            ...
            free: true,
            modelsdev_id: "", // 本地:模型是用户 `ollama pull` 的,无在线目录(用 `ollama list`)
